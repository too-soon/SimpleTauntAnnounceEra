...
-- to focus the player.
local tauntAuras = {
	['Taunt']=1,			-- Warrior, Taunt
	['Mocking Blow']=1,		-- Warrior, Mocking Blow
	['Challenging Shout']=1,	-- Warrior, Challenging Shout
	['Growl']=1,			-- Druid, Growl
	['Challenging Roar']=1,		-- Druid, Challenging Roar
}
...
local tauntSpells = {
	['Growl']=1,			-- Hunter pet, Growl
	['Distracting Shot']=1,		-- Hunter, Distracting Shot
	['Torment']=1,			-- Warlock Voidwalker, Torment
	['Suffering']=1,		-- Warlock Voidwalker, Suffering
}
...
--]]
	...
	elseif subEvent == 'SPELL_CAST_SUCCESS' then
		...
		if spellName == 'Growl' and bit.band(sourceFlags, COMBATLOG_OBJECT_TYPE_PLAYER) > 0 then return end
	elseif subEvent == 'SPELL_MISSED' then
		...
		failString = ' failed (' .. (missType or '?'):lower() .. ')'
	else
	...
	if sourceName == GetPlayerName() then
		...
		-- Announce taunt
		self:AnnounceTaunt(mode, string.format('Taunting %s%s with %s%s', GetRaidIconString(raidIcon, mode), destName or '?', spellName or '?', failString or ''), cfg.soundOwn)
	elseif bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_MINE) > 0 then
		...
		-- Announce taunt
		self:AnnounceTaunt(mode, string.format('My %s taunting %s%s with %s%s', sourceName or '?', GetRaidIconString(raidIcon, mode), destName or '?', spellName or '?', failString or ''), cfg.soundOwn)
	else
		...
		if (inRaid and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_RAID) > 0) or (inGroup and bit.band(sourceFlags, COMBATLOG_OBJECT_AFFILIATION_PARTY) > 0) then
			...
			self:AnnounceTaunt(mode, string.format('%s taunting %s%s with %s%s', name, GetRaidIconString(raidIcon, mode), destName or '?', spellName or '?', failString or ''), cfg.soundOther)
		end
	end
end
...
function frame:Print(...)
	return print('|cFFF0F050STA|r:', ...)
end

-- Formatted print with STA prefix
function frame:Printf(s, ...)
	return print('|cFFF0F050STA|r:', string.format(s, ...))
end
...
function frame:ParseCompactCmdModes(cmd)
	if cmd:len() ~= 7 then
		self:Print('compact mode must contain 6 characters')
		return
	end
...
function frame:OnSlashCmd(cmd, ...)
	...
	elseif cmd == 'help' then
		...
		self:Print('For each status (|cFF20E020solo party|r and |cFF20E020raid|r), own and')
		self:Print('other mode can be set to |cFF20E020off self say party|r or |cFF20E020raid|r.')
		self:Print('Own covers taunts made by you or your pet, other covers')
		self:Print('taunts made by any player or pet in your party/raid.')
		self:Print('Examples:')
		self:Print('  |cFF20E020/sta party|r - print current party modes')
		self:Print('  |cFF20E020/sta solo self|r - when solo, own taunts to yourself')
		self:Print('  |cFF20E020/sta raid say self|r - when raid, own to say, others to self')
	else
		-- By default, show status and options
		self:Print(addonName, addon.version)
		self:Print('Syntax: /sta <command> [options]')
		self:Print('  |cFF20E020on/off|r - turn announcing on/off')
		self:Print('  |cFF20E020solo/party/raid [own [other]]|r - set mode')
